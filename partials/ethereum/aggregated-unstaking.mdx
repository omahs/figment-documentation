...
- Supply an array of `VoluntaryExit` objects, consisting of:
  - `validator_index` - Index of the validator to exit.
  - `epoch` - Earliest epoch when a voluntary exit can be processed for the given validator. This value must be for a past or current epoch, not a future epoch.
