---
title: Staking API Endpoints
sidebar_position: 2
---

import Tabs from "@theme/Tabs";
import TabItem from "@theme/TabItem";
import Link from "@docusaurus/Link";

The Staking API has three endpoints for managing flows.

There are also five endpoints for managing webhooks, which are covered in the <Link to="#managing-staking-api-webhooks">Managing Staking API Webhooks</Link> section.

## Managing Staking API Flows

### Start a New Flow

- `POST` `/api/v1/flows`

#### Parameters

#### Body

| parameter      |  type  | description                                        | required |
| :------------- | :----: | :------------------------------------------------- | -------: |
| `network_code` | String | Network on which this flow operates (ex. `solana`) | Required |
| `chain_code`   | String | Chain on which this flow operates (ex. `mainnet`)  | Required |
| `operation`    | String | The operation to perform (ex. `staking`)           | Required |
| `version`      | String | API version to use (ex. `v1`)                      | Optional |

#### curl Request

```bash title="Example cURL request to the Staking API to start a staking flow on NEAR"
curl -X POST 'https://near-slate.datahub.figment.io/api/v1/flows' \
-H 'Authorization: <api_key>' \
-d '{"flow": {"network_code":"near", "chain_code": "testnet", "operation": "staking", "version": "v1"}}'
```

<details><summary>Click to view sample response</summary>

:::note

In the response, the `actions` array contains the name of the next action in the flow and details of the expected inputs in the `data` field.

:::

```json
{
  "id": "1d9e9d5d-f4a2-4974-b549-c35531344fa9",
  "state": "initialized",
  "actions": [
    {
      "name": "create_delegate_tx",
      "inputs": [
        {
          "name": "delegator_address",
          "display": "Delegator Address",
          "description": "",
          "type": "string",
          "validations": [
            {
              "type": "presence",
              "options": {}
            },
            {
              "type": "account_balance",
              "options": {
                "on": "with_on_chain_validations",
                "greater_than_or_equal_to": null
              }
            }
          ]
        },
        {
          "name": "delegator_pubkey",
          "display": "Delegator Pubkey",
          "description": "",
          "type": "string",
          "validations": [
            {
              "type": "presence",
              "options": {}
            }
          ]
        },
        {
          "name": "validator_address",
          "display": "Validator Address",
          "description": "",
          "type": "string",
          "validations": [
            {
              "type": "presence",
              "options": {}
            },
            {
              "type": "is_validator",
              "options": {
                "on": "with_on_chain_validations"
              }
            }
          ]
        },
        {
          "name": "amount",
          "display": "Amount",
          "description": "",
          "type": "decimal",
          "validations": [
            {
              "type": "presence",
              "options": {}
            },
            {
              "type": "numericality",
              "options": {
                "greater_than": 0
              }
            }
          ]
        },
        {
          "name": "max_gas",
          "display": "Max Gas",
          "description": "",
          "type": "decimal",
          "validations": []
        }
      ]
    }
  ],
  "data": {
    "delegator_address": null,
    "delegator_pubkey": null,
    "validator_address": null,
    "amount": null,
    "max_gas": null,
    "delegate_transaction": null
  },
  "network_code": "near",
  "chain_code": "testnet",
  "created_at": "2022-07-19T17:54:02.882Z",
  "updated_at": "2022-07-19T17:54:02.882Z"
}
```

</details>

---

### Get Status of an Existing Flow

- `GET` `/api/v1/flows/[:flow_id]`

#### Parameters

- `[:flow_id]`: This is a placeholder for the actual Flow ID. Pass the Flow ID you received in the reponse when creating the flow in the GET request URL.

#### curl Request

```bash
curl -X GET 'https://eth-slate.datahub.figment.io/api/v1/flows/[:flow_id]' \
-H "Authorization: <api_key>"
```

<details><summary>Click to see sample response</summary>

:::note

In the response, the `actions` array contains the name of the next action in the flow and the expected inputs for the next step in the `data` field.

:::

```json
{
  "id": "1d9e9d5d-f4a2-4974-b549-c35531344fa9",
  "state": "initialized",
  "actions": [
    {
      "name": "create_delegate_tx",
      "inputs": [
        {
          "name": "delegator_address",
          "display": "Delegator Address",
          "description": "",
          "type": "string",
          "validations": [
            {
              "type": "presence",
              "options": {}
            },
            {
              "type": "account_balance",
              "options": {
                "on": "with_on_chain_validations",
                "greater_than_or_equal_to": null
              }
            }
          ]
        },
        {
          "name": "delegator_pubkey",
          "display": "Delegator Pubkey",
          "description": "",
          "type": "string",
          "validations": [
            {
              "type": "presence",
              "options": {}
            }
          ]
        },
        {
          "name": "validator_address",
          "display": "Validator Address",
          "description": "",
          "type": "string",
          "validations": [
            {
              "type": "presence",
              "options": {}
            },
            {
              "type": "is_validator",
              "options": {
                "on": "with_on_chain_validations"
              }
            }
          ]
        },
        {
          "name": "amount",
          "display": "Amount",
          "description": "",
          "type": "decimal",
          "validations": [
            {
              "type": "presence",
              "options": {}
            },
            {
              "type": "numericality",
              "options": {
                "greater_than": 0
              }
            }
          ]
        },
        {
          "name": "max_gas",
          "display": "Max Gas",
          "description": "",
          "type": "decimal",
          "validations": []
        }
      ]
    }
  ],
  "data": {
    "delegator_address": null,
    "delegator_pubkey": null,
    "validator_address": null,
    "amount": null,
    "max_gas": null,
    "delegate_transaction": null
  },
  "network_code": "near",
  "chain_code": "testnet",
  "created_at": "2022-07-19T17:54:02.882Z",
  "updated_at": "2022-07-19T17:54:02.882Z"
}
```

</details>

---

### Continue to Next Step

- `POST` `/api/v1/flows/[:flow_id]/next`

#### Parameters

##### Body

The parameters you're sending in the `inputs` will be different, depending on which network and flow you are using.
Refer to the individual protocol guides (Avalanche, Ethereum, etc.) for more details. The links are in the navigation menu on the left.

#### curl Request

```bash {1} title="Sample cURL Request (without values)"
curl -X POST https://eth-slate.datahub.figment.io/api/v1/flows/[:flow_id]/next \
-H "Authorization: <api_key>" \
-d '{"name": "create_deposit_transaction", \
  "inputs": {
    "funding_account_address": "",
    "validator_pub_key": "",
    "withdrawal_credentials": "",
    "signature": "",
    "deposit_data_root": ""
  }
}'
```

<details><summary>Click to see sample response</summary>

:::note

In the response, the `actions` array contains the name of the next action in the flow and the expected `inputs`.

:::

```json {4,6}
{
  "id": "1d9e9d5d-f4a2-4974-b549-c35531344fa9",
  "state": "initialized",
  "actions": [
    {
      "name": "create_delegate_tx",
      "inputs": [
        {
          "name": "delegator_address",
          "display": "Delegator Address",
          "description": "",
          "type": "string",
          "validations": [
            {
              "type": "presence",
              "options": {}
            },
            {
              "type": "account_balance",
              "options": {
                "on": "with_on_chain_validations",
                "greater_than_or_equal_to": null
              }
            }
          ]
        },
        {
          "name": "delegator_pubkey",
          "display": "Delegator Pubkey",
          "description": "",
          "type": "string",
          "validations": [
            {
              "type": "presence",
              "options": {}
            }
          ]
        },
        {
          "name": "validator_address",
          "display": "Validator Address",
          "description": "",
          "type": "string",
          "validations": [
            {
              "type": "presence",
              "options": {}
            },
            {
              "type": "is_validator",
              "options": {
                "on": "with_on_chain_validations"
              }
            }
          ]
        },
        {
          "name": "amount",
          "display": "Amount",
          "description": "",
          "type": "decimal",
          "validations": [
            {
              "type": "presence",
              "options": {}
            },
            {
              "type": "numericality",
              "options": {
                "greater_than": 0
              }
            }
          ]
        },
        {
          "name": "max_gas",
          "display": "Max Gas",
          "description": "",
          "type": "decimal",
          "validations": []
        }
      ]
    }
  ],
  "data": {
    "delegator_address": null,
    "delegator_pubkey": null,
    "validator_address": null,
    "amount": null,
    "max_gas": null,
    "delegate_transaction": null
  },
  "network_code": "near",
  "chain_code": "testnet",
  "created_at": "2022-07-19T17:54:02.882Z",
  "updated_at": "2022-07-19T17:54:02.882Z"
}
```

</details>

### Code Examples for Managing Flows

This code assumes you are not using a proxy, and so it will pass the `Authorization` header.

In production, you will want to use a proxy. Follow our guide <Link to="/guides/proxy-requests-to-figment-apis">Proxy Requests to Figment APIs</Link> for more information.

<Tabs><TabItem value="js" label="JavaScript">

<details><summary>Click to view sample code</summary>

```javascript
const NETWORK_CODES = {
  avalanche: "avalanche",
  cosmos: "cosmos",
  ethereum: "ethereum",
  near: "near",
  polkadot: "dot",
  polygon: "polygon",
  solana: "solana",
};

const CHAIN_CODES = {
  mainnet: "mainnet", // All Networks
  goerli: "goerli-prater", // Ethereum PoS testnet
  testnet: "testnet", // NEAR testnet, Avalanche Fuji
  devnet: "devnet", // Solana devnet
  westend: "westend", // Polkadot testnet
};

const OPERATIONS = {
  staking: "staking", // Ethereum only supports staking
  unstaking: "unstaking", // NEAR or Polkadot
  transfer: "transfer", // NEAR or Polkadot or Solana or Cosmos
  sol_unstaking: "unstaking", // Solana unstaking
  add_proxy: "add_proxy", // Polkadot only
  remove_proxy: "remove_proxy", // Polkadot only
};

const API_KEY = "YOUR API KEY";

// flowUrl would be the Figment API endpoint for the network you're using
// i.e., https://eth-slate.datahub.figment.io/api/v1/flows/
function startNewFlow(network, chain, operation, params = {}) {
  fetch(flowUrl, {
    method: "POST",
    headers: {
      Authorization: `${API_KEY}`,
      "Content-Type": "application/json",
    },
    body: JSON.stringify({
      flow: {
        network_code: NETWORK_CODES[`${network}`],
        chain_code: CHAIN_CODES[`${chain}`],
        operation: OPERATIONS[`${operation}`],
        version: "v1",
      },
    }),
  })
    .then((result) => result.json())
    .then(console.log);
}

function continueFlow(flowId, name, inputs = {}) {
  console.log(name, inputs);
  fetch(`${flowUrl}/${flowId}/next`, {
    method: "PUT",
    headers: {
      Authorization: `${API_KEY}`,
      "Content-Type": "application/json",
    },
    body: JSON.stringify({
      name,
      inputs,
    }),
  })
    .then((result) => result.json())
    .then((json) => console.log(JSON.stringify(json, null, 2)));
}

function getFlow(flowId) {
  fetch(`${flowUrl}/${flowId}`, {
    method: "GET",
    headers: {
      Authorization: `${API_KEY}`,
    },
  })
    .then((result) => result.json())
    .then((json) => console.log(JSON.stringify(json, null, 2)));
}
```

</details>

</TabItem></Tabs>

## Managing Staking API Webhooks

### Authentication

You require an API key to authorize requests to the Webhooks API endpoint.
Refer to the <Link to="/quickstart/api-authentication">API Authentication</Link> quick start for details.

```bash {2} title="Sample cURL Request"
curl -X GET 'https://eth-slate.figment.io/api/v1/webhook_endpoints/15' \
-H 'Authorization: <api_key>'
```

### Create a new Webhook endpoint

Send a `POST` request to `/api/v1/webhook_endpoints` to create a new webhook.

The Staking API will send a webhook payload to the specified `target_url` when an event is triggered (undelegation, etc.)

:::info note

Each webhook is identified by a sequential number.
Currently when a webhook ID has been deleted it cannot be reused.

:::

Specify the `target_url`, where your webhook server is located.

```bash {6} title="Sample cURL Request"
curl -X POST 'https://near-slate.figment.io/api/v1/webhook_endpoints' \
-H 'Authorization: <api_key>' \
-H 'Content-Type: application/json' \
-d '{
    "webhook_endpoint": {
        "target_url": "https://example.com/my/endpoint",
        "event_types": ["*"],
        "enabled": "true"
    }
}'
```

You will only be provided with the `secret` in the response when you create a webhook.
Remember to capture this data. Learn more about webhook signature verification in the <Link to="#references">References</Link> section.

```json {8} title="Sample Response - The value of secret has been truncated for display."
{
  "id": 16,
  "event_types": [],
  "target_url": "https://example.com/my/endpoint",
  "enabled": true,
  "object": "webhook_endpoint",
  "created": "2022-07-26T19:27:12.647Z",
  "secret": "whsec_B8sidV4Bm8NnstMuP3a9TSGYov..."
}
```

### Retreive a Webhook endpoint

You can send a `GET` request, including the webhook ID number in the request URL, to retreive details for an existing webhook endpoint.

```bash title="Sample cURL Request"
curl -X GET 'https://eth-slate.figment.io/api/v1/webhook_endpoints/15' \
-H 'Authorization: <api_key>'
```

```json title="Sample Response"
{
  "object": "list",
  "url": "/api/v1/webhook_endpoints",
  "has_more": false,
  "data": [
    {
      "id": 15,
      "event_types": ["*"],
      "target_url": "https://example.com/my/endpoint",
      "enabled": true,
      "object": "webhook_endpoint",
      "created": "2022-07-26T18:20:21.849Z"
    }
  ]
}
```

### Update Webhook endpoint

You can send a `PUT` request to update details for an existing webhook endpoint.

```bash {8} title="Sample cURL Request"
curl -X PUT 'https://eth-slate.figment.io/api/v1/webhook_endpoints/15' \
-H 'Authorization: <api_key>' \
-H 'Content-Type: application/json' \
-d '{
    "webhook_endpoint": {
        "target_url": "https://example.com/my/new/endpoint",
        "event_types": ["*"],
        "enabled": "false"
    }
}'
```

```json {5} title="Sample Response"
{
  "id": 15,
  "event_types": ["*"],
  "target_url": "https://example.com/my/new/endpoint",
  "enabled": false,
  "object": "webhook_endpoint",
  "created": "2022-07-26T19:27:12.647Z"
}
```

### List existing Webhook endpoints

You can send a `GET` request to retreive details for all your webhook endpoints.

```bash title="Sample cURL Request"
curl -X PUT 'https://eth-slate.figment.io/api/v1/webhook_endpoints' \
-H 'Authorization: <api_key>'
```

```json title="Sample Response"
{
  "object": "list",
  "url": "/api/v1/webhook_endpoints",
  "has_more": false,
  "data": [
    {
      "id": 16,
      "event_types": ["*"],
      "target_url": "https://example.com/my/endpoint",
      "enabled": true,
      "object": "webhook_endpoint",
      "created": "2022-07-26T19:27:12.647Z"
    },
    {
      "id": 15,
      "event_types": ["*"],
      "target_url": "https://example.com/my/new/endpoint",
      "enabled": false,
      "object": "webhook_endpoint",
      "created": "2022-07-26T18:20:21.849Z"
    }
  ]
}
```

### Delete an existing Webhook endpoint

You can send a `DELETE` request, including the webhook ID number in the request URL, to delete a webhook endpoint.

```bash title="Sample cURL Request"
curl -X PUT 'https://eth-slate.figment.io/api/v1/webhook_endpoints/15' \
-H 'Authorization: <api_key>'
```

```text title="Sample Response"
204 - No Content
```

:::info

This response does not contain any JSON. Status Code **204** indicates a successful deletion.

:::
