...
//
const FLOW_ID = "3e079c15-befb-4bd2-bdf9-f3e7eaeb182d";
...
async function handleRequest(request) {
  ...
  try {
    if (data) {
      // Different events will generate different payload structures
      // All Staking API payloads should include a "flow_id"
      // If the key "error" is present, the "event_type" should end with .error
      // Staking API Webhooks can be set to filter by event type, making it easier to
      // handle only the events you care about.
      //
      // Handling differing payload structures in a Staking API webhook payload,
      // we can iterate over the existing keys to see what we're dealing with
      for (const [key, value] of Object.entries(data)) {
        console.log(`${key}: ${value}`);
      }

      // Display the Headers key/value pairs
      console.log("Request Headers:\n");
      for (var pair of request.headers.entries()) {
        console.log(pair[0] + ": " + pair[1]);
      }

      // Always have a configured webhook secret!
      // Validate we received a request with this secret efore proceeding any further.
      if (headers.get("staking-webhook-auth") !== WEBHOOK_SECRET) {
        return new Response(
          "Incorrect webhook secret. Ensure that your staking-webhook-auth header is set to the appropriate value.",
          {
      ...
      // Return the incoming payload as a response.
