...
In addition having a middleware means we can take further steps such as validating CORS Origins, whitelisting IPs, throttling requests and more.
...
- `PROXY_ORIGIN` - The the URL where the worker is deployed e.g. `https://proxy.yourusername.workers.dev`. If you have set up a custom domain in Cloudflare, use that instead.
...
<details><summary>Get Ethereum gas price from Node API</summary>
...
<details><summary>Get Ethereum rewards for a Validator from Rewards API</summary>
...
<details><summary>Get Avalanche transactions from Transaction Search API</summary>
