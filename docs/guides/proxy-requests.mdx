...
provide a foundational middleware for further security controls. This guide assumes working knowledge of JavaScript and a <Link to="https://cloudflare.com/">Cloudflare</Link> account.
...
Some examples given the proxy `http://api-proxy.roganx.worker.dev`:

<details><summary>Get Arbitrum block number from Node API</summary>

```javascript
fetch("http://api-proxy.roganx.worker.dev/node-api/arbitrum", {
  method: "POST",
