...
import img9 from "@site/static/img/guides/proxy-requests/cloudflare_request.png";
...
- Add an environment variable named `API_KEY`. Click the "Encrypt" button once you've set the value of `API_KEY`. The key will be hidden in the Cloudflare UI, but still accessible within the worker.
- Add an environment variable named `ALLOWED_ORIGINS`. The values must be enclosed in quotes, in an array (`["docs.figment.io"]`). Requests from outside of a web browser will not include an `Origin`. Include the value `null`, without quotes if you intend to make requests from a non-browser environment (`["docs.figment.io", null]`) .
...
4. Click on the "Quick edit" button, then copy and paste the code below into the code editor on the left side of the page.
...
<details><summary>Click here to view the full example proxy, ≈200 lines of code. Each function has JSDoc comments for convenience.</summary>
...
<details><summary>Click here to view details about the environment variables</summary>
...
`HOSTS` separates `wss` and `https` hostnames, and places them into each applicable service. Figment provides access to infrastructure that is managed by a world class DevOps team. One such API is the Node API, allowing clients to communicate directly with Proof-of-Stake networks.

Hence, `env.HOSTS['wss']['node-api'][0]` would refer to the Arbitrum hostname.
:::
...
Cloudflare's Service Worker API uses event listeners to handle the request with a callback function.
In this case, we want to tie our functionality together and ensure it is used to direct the request to the appropriate function.
...
3. Send a request to the proxy in Cloudflare:
   ...
   - In the case of the example shown below, we would also need to append `/solana/node-api` to the request URL.
