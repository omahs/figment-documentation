...
slug: /guides/staking-api/polygon/claim-rewards/submit-claim-rewards-data
---
...
- `inputs` &mdash; `object`
  ...
  - `gas_limit` &mdash; `number` Adjust gas limit for the claim transaction .
  - `gas_price` &mdash; `number` Adjust gas limit for the claim transaction .

### Sample cURL Request

```bash {5-8}
curl -X PUT 'https://matic-slate.datahub.figment.io/api/v1/flows/21ec6775-720c-4c58-bfd2-73275200e1d0/next' \
